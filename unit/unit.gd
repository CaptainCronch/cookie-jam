...
var enemies_to_hit: Array[Enemy] = []
...
func check_interaction() -> void:
	for area in separate_area.get_overlapping_areas():
		var parent := area.get_parent()
		if parent is Enemy and parent == targeted_enemy and interact_timer <= 0.0:
			(parent as Enemy).hurt(self, damage)
			...
			if area is Bank and not current_item == ITEM.NONE and not current_item == ITEM.BODY:
				...
				interacting = true
				animator.play("interact")
			elif area == current_goal and current_item == ITEM.NONE:
